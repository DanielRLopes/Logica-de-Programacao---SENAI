...
A ) 
*/
let userName = readlineSync.question(`Insira seu Nome:`)
let userAge = readlineSync.questionInt(`Insira sua Idade:`)
let anosExperiencia = readlineSync.questionInt(`Insira quantos anos de experiencia tem:`)
let possuiSuperior = readlineSync.keyInYN(`Possui Ensino Superior? `)
let possuiTecnico = readlineSync.keyInYN(`Possui curso Tecnico? `)
let resideNoBrasil = readlineSync.keyInYN(`Reside no Brasil? `)

const candidato = {
    nome: userName,
    idade: userAge,
    anosExperiencia: anosExperiencia,
    ensinoSuperior: possuiSuperior,
    cursoTecnico: possuiTecnico,
    resideBrasil: resideNoBrasil
}
...
/* 
Bati a cabeça era só criar a PROPRIEDADE INTEIRA FORA DO OBJETO,
Assim não dando erro de inicialização por chamada dentro do próprio.
*/
criteriosAprovacao.candidatoAprovado = criteriosAprovacao.idadeValida && criteriosAprovacao.experienciaSuficiente && criteriosAprovacao.formacaoAdequada && candidato.resideBrasil
...
let temperatura = readlineSync.questionFloat(`Insira a temperatura atual em graus Celsius:`)
...
const classificao = {
    temperatura: {
        muitoFrio: temperatura < 10,
        frio: temperatura >= 10 && temperatura < 18,
        agradavel: temperatura >= 18 && temperatura <= 25,
        quente: temperatura > 25 && temperatura <= 35,
        muitoQuente: temperatura > 35
    }
}
...
let estaEmAmbienteInterno = readlineSync.keyInYN(`Esta em um ambiente interno?`)

/* 
D ) 
*/

classificao.precisaArCondicionado = {
    verificacao: temperatura > 28 && !estaEmAmbienteInterno ? "Nao, porque ja esta fora" : "Sim, precisa de ar condicionado"
}
...
console.log(`
    ${classificao.precisaArCondicionado.verificacao}
    `)
